...
model = load_model("battery_life_model.h5", compile=False)  # Ensure you save the model as .h
...
def predict_battery_life(type_discharge, Capacity, Re, Rct, label_encoder, scaler, model):
    ...
    print(predicted_battery_life.shape)
...
if st.button('Predict Battery Life'):
    ...
    st.success(f"Predicted Battery Life: {predicted_battery_life.item():.2f} units")
